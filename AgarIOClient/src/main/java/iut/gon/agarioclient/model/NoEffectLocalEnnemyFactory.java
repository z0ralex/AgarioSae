// NoEffectLocalEnnemyFactory.java
...
public class NoEffectLocalEnnemyFactory implements LocalEnnemyFactory {
    ...
    public List<Ennemy> generate(int quantity) {
        ...
        for (int i = 0; i < quantity; i++) { // Strat Random. ATTENTION Remettre Quantity / 3
            Random r = new Random();
            Point2D p = new Point2D(r.nextInt(786, 986), r.nextInt(375, 575));
            list.add(new Ennemy(UUID.randomUUID().toString(), p, 10, new IAStratRandomMoving(), 5.0));
        }
        ...
        for (int i = 0; i < quantity / 3; i++) { // Strat Pellets
            ...
            list.add(new Ennemy(UUID.randomUUID().toString(), p, 10, new IAStratEatPelletsOnly(root), 5.0));
        }
        ...
        for (int i = 0; i < quantity / 3; i++) { // Strat Players
            ...
            Point2D p = new Point2D(r.nextInt(80), r.nextInt(80));
            list.add(new Ennemy(UUID.randomUUID().toString(), p, 10, new IAStratEatPlayers(), 5.0)); // Added speed parameter
        }
